...
def setup_confluence_rag(update=False):
    ...
    index_name = "./faiss_confluence_index"
    
    if update and os.path.exists(index_name):
        # Update existing vector store
        vectordb = FAISS.load_local(index_name, embeddings)
        ...
        # Create vector store using FAISS (no SQLite dependency)
...
def update_confluence_vector_db(vectordb, space_key=None, page_id=None):
    ...
    if new_documents:
        ...
        # Save the updated index
        vectordb.save_local("./faiss_confluence_index")
...
if __name__ == "__main__":
    ...
    print("Setting up Confluence RAG system with FAISS...")
